...
impl TryFrom<AvailableCategory> for DataRules {
    ...
    fn try_from(category: AvailableCategory) -> Result<Self, serde_yaml::Error> {
        let content = match category {
            AvailableCategory::INCLUSION => include_str!("rules/inclusion_rules.yaml"),
            AvailableCategory::PII => include_str!("rules/pii_rules.yaml"),
        };
        let data_rules = serde_yaml::from_str(content)?;
        Ok(data_rules)
    }
}
